...
class Tiers:
    def __init__(self, store: Store):
        self.store = store
        self.universal = ["You are one temporary kernel holding a persistent role.",
    ...
    def assemble(self, role: Role, task: Task, predecessor: dict | None = None) -> str:
        payload = {"facts": self.universal, "memory": [], "task": asdict(task)}
        ...
        for kind in ("paths", "names", "hosts"):
            for needle in getattr(role.deny, kind):
                if needle.casefold() in rendered.casefold():
                    raise Denied(f"brief refused: denied {kind[:-1]} leaked: {needle}")
        return rendered

    def write(self, tier: int, kind: str, body: Any, by: str, kernel: str) -> None:
        self.store.append(f"state/tier-{tier}.jsonl", {"at": datetime.now(timezone.utc).isoformat(),
