...
def load_local_oauth_credentials() -> dict[str, dict[str, Any]]:
    ...
    Checks ~/.arity/auth.json first, then imports from ~/.omp/agent/agent.db.
    """
    creds: dict[str, dict[str, Any]] = {}

    # 1. Check standalone arity auth store
    arity_auth_file = Path.home() / ".arity" / "auth.json"
    if arity_auth_file.exists():
        try:
            creds.update(json.loads(arity_auth_file.read_text(encoding="utf-8")))
        except Exception:
            pass
