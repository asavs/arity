...
The distribution, primary CLI, Python namespace, module entry point, state/config location,
and environment settings now use Arity names exclusively. Compatibility aliases, fallback
locations, fallback settings, and automatic state migration are intentionally absent.
Historical entries below use the current canonical project name while retaining their original
dates, behavior, and release scope.
...
1. **Arity is the public distribution and control surface**
   ...
   - The Python namespace, console entry point, module entry point, state directory, and
     environment settings share the `arity`/`ARITY` name without legacy fallbacks.
...
a front door, and uses it to build one of Arity's own organs.

### What changed:

1. **`arity race` — one task, N candidates, one archivist (`arity/race.py`, `arity/terrarium.py`)**
...
2. **`arity run` — the front door (`arity/race.py`, `arity/cli.py`)**
   - One seat per model, fullest quota first, wire-capable first, capped by `--candidates` / `ARITY`.
...
3. **The archivist counts; the judge opines; you decide (`arity/archivist.py`, `arity/standings.py`)**
   ...
   - `arity standings [--by model|signature|harness]` aggregates those records — success and hidden-pass
...
4. **Types: a role plus a language (`arity/definitions/types/`, `arity/roles.py`)**
...
5. **Task bank, tester, judge, secretary (`arity/tasks.py`, `arity/definitions/`)**
   - `arity tasks`: briefs with hidden acceptance tests (`lru_cache` with a 200k-ops time budget,
...
6. **Seats, wires, and metering (`arity/ledger.py`, `arity/wire.py`, `arity/handlers.py`)**
...
7. **Stores (`arity/stores/sqlite.py`)**
   - `SqliteRecordStore` implements the `RecordStore` seam (append/query) plus `kinds()` and idempotent
     `replay_jsonl()`. `ARITY_STORE=sqlite` selects it; the JSONL store (now locked against interleaved
     writes) remains the default.
   - **It was built by the trial system it records:** `arity run --task sqlite_record_store --tester
